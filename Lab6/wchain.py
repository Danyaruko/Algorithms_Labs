...
def find_max_wchain(vocabulary):
    ...
    word_wchain_max_len_map = {word: 1 for word in vocabulary}
    for word in vocabulary:
        for i in range(len(word)):
            new_word = word[:i]+word[i+1:]
            if new_word in vocabulary:
                word_wchain_max_len_map[word] = max(
                    word_wchain_max_len_map[word], 1 + word_wchain_max_len_map[new_word])
    return max(word_wchain_max_len_map.values())
